// app/api/carparkAvailabilityv2/route.ts
import { NextResponse } from "next/server";

export async function GET(request: Request) {
  // Read the $skip parameter from the query string, default to "0"
  const { searchParams } = new URL(request.url);
  const skip = searchParams.get("$skip") || "0";

  // Use the corrected endpoint URL and your API key from the environment.
