...
const EateriesFilter = () => {
  ...
  const cuisines =
    mall?.eateries?.map((e) => e.cuisine_type).filter(Boolean) ?? [];
  const uniqueCuisines = Array.from(new Set(cuisines));

  const filteredEateries = mall?.eateries.filter((e) => {
      ...
      {/* Eateries */}
          <div className="grid md:grid-cols-3 gap-4">
            {filteredEateries.map((eatery) => {
                console.log("Eatery:", eatery.name);
                console.log("Mall Map URL:", mall.google_maps_url);
                ...
                >
                  <div className="relative w-full h-52">
                    ...
                    {/* Logo - clickable if website */}
                      ...
                      >
                        ⭐ {eatery.rating} | <span className="text-xs bg-gray-700 text-white rounded px-2 py-0.5">
                      ...
                      {eatery.rating >= 4.5 && eatery.total_reviews > 500 && (
                        ...
                        <p className="text-sm">📍 Level {eatery.floor}-#{eatery.unit}</p>
                    ...
                    >
                      {bookmarked.includes(eatery.id) ? <FaHeart /> : <FaRegHeart />}
                    </button>
                  </div>
